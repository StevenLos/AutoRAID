# AutoRAID — Mac Usage Guide

Day-to-day operation of the Automated Meetings & Email Notes → RAID Log pipeline on macOS. Setup must be complete before using this guide — see [SETUP-Mac.md](SETUP-Mac.md).
...
2. Drag them directly from Outlook into the `Email/Originals/` folder in Finder.

Outlook for Mac saves dragged messages as `.eml` files — the expected input format for the Mac pipeline.

### Step 2 — Convert EML to Markdown

Run the ingest script manually from Terminal:

```zsh
<vault-root>/Email/Scripts/run_email_ingest.sh
```

Or call the Python script directly:

```zsh
python3 "<vault-root>/Email/Scripts/eml_to_markdown.py" \
    "<vault-root>/Email/Originals" \
    "<vault-root>/Email/Inbox" \
    "<vault-root>/Email/Attachments"
```

If you configured launchd automation during setup, this step runs automatically and can be skipped.

Each run will:
- Convert new `.eml` files to `.md` notes in `Email/Inbox/`
...
```zsh
python3 "<vault-root>/Email/Scripts/vtt_to_markdown.py" \
...
1. Drag from Outlook → `Email/Originals/` in Finder
2. Run `run_email_ingest.sh` (or wait for launchd)
...
**No messages selected (drag produces nothing)**
Ensure you are dragging from the Outlook message list, not from an open message window.
